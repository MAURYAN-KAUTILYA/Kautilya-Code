...
export type ThemeAccentId = "classic" | "green-yellow" | "pink" | "blue";
...
export interface AppThemeTokens {
  ...
  textTertiary: string;
  builderShellBackground: string;
  builderRailBackground: string;
  builderTopbarBackground: string;
  builderGlass: string;
  builderGlassStrong: string;
  builderSidebarBackground: string;
  builderBorder: string;
  builderBorderSubtle: string;
  builderMuted: string;
  builderMutedStrong: string;
  builderMutedSoft: string;
  builderInverseBackground: string;
  builderInversePanel: string;
  builderInverseBorder: string;
  builderInverseText: string;
  builderInverseMuted: string;
...
interface AppThemeContextValue {
  ...
  setAccentId: (accentId: ThemeAccentId) => void;
}

type AccentModeConfig = {
  accent: string;
  accentStrong: string;
  accentAlt: string;
  accentRgb: string;
...
type AccentFamily = {
  label: string;
  description: string;
  light: AccentModeConfig;
  dark: AccentModeConfig;
};
...
const ACCENT_FAMILIES: Record<ThemeAccentId, AccentFamily> = {
  classic: {
    label: "Classic",
...
export const THEME_ACCENT_OPTIONS: ThemeAccentOption[] = (
  Object.entries(ACCENT_FAMILIES) as Array<[ThemeAccentId, AccentFamily]>
).map(([id, accent]) => ({
  id,
  label: accent.label,
...
export function buildThemeTokens(preference: ThemePreference): AppThemeTokens {
  ...
  const builderInverseBackground =
    "linear-gradient(180deg, rgba(17, 20, 27, 0.98) 0%, rgba(10, 12, 18, 1) 100%)";
...
function applyThemeToDocument(tokens: AppThemeTokens) {
