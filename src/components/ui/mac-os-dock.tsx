...
const glassSurface: React.CSSProperties = {
  background:
    'radial-gradient(circle at top, rgba(255,255,255,0.22), transparent 66%), linear-gradient(180deg, rgba(255,255,255,0.14) 0%, rgba(255,255,255,0.05) 100%), var(--builder-glass-strong)',
  border: '1px solid rgba(255,255,255,0.14)',
  boxShadow:
    '0 14px 34px rgba(0,0,0,0.18), 0 6px 16px rgba(0,0,0,0.14), inset 0 1px 0 rgba(255,255,255,0.18), inset 0 -1px 0 rgba(0,0,0,0.08)',
...
const MacOSDock: React.FC<MacOSDockProps> = ({
  ...
  const scaleStateRef = useRef<number[]>(apps.map(() => 1));
  const positionStateRef = useRef<number[]>([]);
  ...
  const animateToTarget = useCallback(() => {
    ...
    const nextScales = scaleStateRef.current.map((currentScale, index) => {
      ...
      if (Math.abs(nextScale - targetScales[index]) > 0.002) scalesNeedUpdate = true;
      return nextScale;
    });

    const nextPositions = positionStateRef.current.map((currentPosition, index) => {
      const nextPosition =
        currentPosition + (targetPositions[index] - currentPosition) * lerpFactor;
      if (Math.abs(nextPosition - targetPositions[index]) > 0.1) positionsNeedUpdate = true;
      return nextPosition;
    });
  ...
  const handleDockAppClick = (appId: string, index: number) => {
    if (iconRefs.current[index]) {
      createBounceAnimation(iconRefs.current[index] as HTMLElement);
    }
    ...
    currentPositions.length > 0
      ? Math.max(
          ...currentPositions.map(
            (position, index) =>
              position + (baseIconSize * currentScales[index]) / 2,
          ),
        )
              ...
              {apps.map((app, index) => {
                ...
                return (
                  <button
                    ...
                    title={app.name}
                    aria-label={app.name}
                    onClick={() => handleDockAppClick(app.id, index)}
                    className="absolute flex items-end justify-center border-0 bg-transparent p-0"
                    style={{
                  ...
                  >
                    <span
                      ...
                      style={{
                        ...
                        background:
                          'radial-gradient(circle at top, rgba(255,255,255,0.16), transparent 62%), linear-gradient(180deg, rgba(255,255,255,0.16) 0%, rgba(255,255,255,0.04) 100%), rgba(18,18,20,0.74)',
                        border: '1px solid rgba(255,255,255,0.1)',
                        boxShadow: `0 ${Math.max(3, baseIconSize * 0.06)}px ${Math.max(
                          10,
                          baseIconSize * 0.2,
                        )}px rgba(0,0,0,0.24), inset 0 1px 0 rgba(255,255,255,0.08)`,
