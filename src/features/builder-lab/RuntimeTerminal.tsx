...
interface RuntimeTerminalProps {
  ...
  onConsoleEntry: (entry: ConsoleLine) => void;
}
...
export default function RuntimeTerminal({
  ...
  const statusRef = useRef<"idle" | "running" | "error">("idle");
  ...
  const syncStatus = (next: "idle" | "running" | "error") => {
    statusRef.current = next;
    setStatus(next);
  };

  const appendConsole = (level: ConsoleLine["level"], text: string, source: ConsoleLine["source"] = "runtime") => {
  ...
  const handleRuntimeEvent = (payload: any) => {
    const term = terminalRef.current;
    if (!term || !payload || typeof payload !== "object") return;
  ...
  const streamCommand = async (command: string) => {
    ...
    if (!term || !trimmed || statusRef.current === "running") return;
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 10, color: "var(--builder-inverse-muted)", letterSpacing: "0.1em" }}>XTERM</span>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <button
              onClick={() => {
                if (!lastCommandRef.current || statusRef.current === "running") return;
            ...
            <button
              ...
              disabled={!activeCommandIdRef.current}
              style={{ background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.3)", color: "#FCA5A5", borderRadius: 8, padding: "6px 10px", fontSize: 10, cursor: !activeCommandIdRef.current ? "not-allowed" : "pointer", opacity: !activeCommandIdRef.current ? 0.5 : 1 }}
            >
              STOP
            </button>
          </div>
      ...
      <div style={{ flex: 1, minHeight: 0, padding: "8px 10px", background: "#09111b" }}>
